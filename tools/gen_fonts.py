...
Three faces, all hand-authored here as '#' bitmaps:
  ...
  bold  body thickened one pixel sideways (buttons, titles, DRAW at 2x and up)
...
def build(name, glyphs, rows, base, space, spacing, bold=False):
    cells = {}
    ...
    for ch, (px, py, w) in placed.items():
        for ry, r in enumerate(cells[ch]):
            for rx, c in enumerate(r):
                if c == "#":
                    img.putpixel((px + rx, py + ry), (255, 255, 255, 255))
    img.save(os.path.join(OUT, name + ".png"))
    lines = [
        'info face="%s" size=%d bold=%d italic=0 charset="" unicode=1 stretchH=100 smooth=0 aa=1 padding=0,0,0,0 spacing=1,1 outline=0'
        % (name, line_h, 1 if bold else 0),
        "common lineHeight=%d base=%d scaleW=%d scaleH=%d pages=1 packed=0 alphaChnl=0 redChnl=4 greenChnl=4 blueChnl=4"
        % (line_h, base, page_w, page_h),
    ...
    for ch in sorted(placed, key=ord):
        ...
        lines.append(
            ...
            % (ord(ch), px, py, w, rows, w + spacing)
        )
    with open(os.path.join(OUT, name + ".fnt"), "w") as f:
        f.write("\n".join(lines) + "\n")
    print("font %-5s %3d glyphs, line %d px, page %dx%d" % (name, len(placed), line_h, page_w, page_h))
